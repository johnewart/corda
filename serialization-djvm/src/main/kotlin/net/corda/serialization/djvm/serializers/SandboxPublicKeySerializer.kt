...
class SandboxPublicKeySerializer(
    ...
    private val decoder: Function<ByteArray, out Any?> = Function<ByteArray, Array<out Any?>> { bits ->
       Crypto.decodePublicKey(bits).run {
           arrayOf(algorithm, format, encoded)
       }
    }.andThen(basicInput).andThen(taskFactory.apply(PublicKeyWrapper::class.java))
